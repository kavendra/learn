...
class InvoiceGeneratorBulk
{
    ...
    protected function getSavePath(Conference $conference)
    {
        ...
        return storage_path( "{$this->storagePath}/".date('Y-m-d').'/'. $this->getFileName($conference) );
    }
    ...
    protected function injectAdditionalData($mergeData, Conference $conference)
    {
        ...
        $conference_brands_array = $conference->brands ? $conference->brands->pluck('id')->toArray()) : '';
        $conference_managers = '';
